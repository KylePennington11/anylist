...
class AnyList extends EventEmitter {
   ...
   */
	async getLists(refreshCache = true) {
		...
		// Also extract stores when loading lists
		this.stores = [];
		if (decoded.shoppingListsResponse && decoded.shoppingListsResponse.listResponses) {
			for (const listResponse of decoded.shoppingListsResponse.listResponses) {
				if (listResponse.stores) {
					for (const store of listResponse.stores) {
						// Avoid duplicates by checking if store already exists
						if (!this.stores.some(s => s.identifier === store.identifier)) {
   ...
   * @return {Promise<Recipe[]>} recipes
	*/
	...
	/**
   * Load all stores from account into memory.
   * @return {Promise<Store[]>} stores
   */
	async getStores(refreshCache = true) {
		const decoded = await this._getUserData(refreshCache);

		// Extract stores from all list responses
		this.stores = [];
		if (decoded.shoppingListsResponse && decoded.shoppingListsResponse.listResponses) {
			for (const listResponse of decoded.shoppingListsResponse.listResponses) {
				if (listResponse.stores) {
   ...
   * @param {string} identifier store ID
   * @return {Store} store
   */
	getStoreById(identifier) {
		return this.stores.find(s => s.identifier === identifier);
	}
